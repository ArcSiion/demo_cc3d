...
void vectime_transpose_boundary(double* A, int NX, int NY, int NZ, int T) {
	...
	for ( tt = 0; tt <= T - VECLEN; tt += VECLEN){	
        ...
        //对Y 轴的前后两个面做转置，以此避免我们再est零散的捡起数据
        for ( x = XSTART ; x <= NX + XSTART - STRIDE * VECLEN ; x ++){

            y = YSTART - YSLOPE;
			for ( z = ZSTART ; z <= NZ + ZSTART - VECLEN; z += VECLEN){	
                vload(v_center_0, B[t%2    ][x + XSLOPE + 1 + STRIDE *3][y][z]);
				vload(v_center_1, B[(t+1)%2][x + XSLOPE + 1 + STRIDE *2][y][z]);
				vload(v_center_2, B[t%2    ][x + XSLOPE + 1 + STRIDE *1][y][z]);
				vload(v_center_3, B[(t+1)%2][x + XSLOPE + 1            ][y][z]);

                vload(v_x_plus_1 , B[t%2    ][x + XSLOPE + 1 + STRIDE *3][y + NY + YSLOPE][z]); //y = NY + YSTART
				vload(v_x_minus_1, B[(t+1)%2][x + XSLOPE + 1 + STRIDE *2][y + NY + YSLOPE][z]);
				vload(v_y_plus_1 , B[t%2    ][x + XSLOPE + 1 + STRIDE *1][y + NY + YSLOPE][z]);
				vload(v_y_minus_1, B[(t+1)%2][x + XSLOPE + 1            ][y + NY + YSLOPE][z]);

				transpose(v_center_0, v_center_1, v_center_2, v_center_3, in, out);
				transpose(v_x_plus_1, v_x_minus_1, v_y_plus_1, v_y_minus_1, in, out);   //y = NY + YSTART

				vstore( B[t%2    ][x + XSLOPE + 1 + STRIDE *3][y][z], v_center_3);
				vstore( B[(t+1)%2][x + XSLOPE + 1 + STRIDE *2][y][z], v_center_2);
				vstore( B[t%2    ][x + XSLOPE + 1 + STRIDE *1][y][z], v_center_1);
				vstore( B[(t+1)%2][x + XSLOPE + 1            ][y][z], v_center_0);

				vstore( B[t%2    ][x + XSLOPE + 1 + STRIDE *3][y + NY + YSLOPE][z], v_y_minus_1);   //y = NY + YSTART
				vstore( B[(t+1)%2][x + XSLOPE + 1 + STRIDE *2][y + NY + YSLOPE][z], v_y_plus_1);
				vstore( B[t%2    ][x + XSLOPE + 1 + STRIDE *1][y + NY + YSLOPE][z], v_x_minus_1);
				vstore( B[(t+1)%2][x + XSLOPE + 1            ][y + NY + YSLOPE][z], v_x_plus_1);
			}
            ...
            for ( y = YSTART; y < NY + YSTART; y++ ) {
                ...
                for ( z = ZSTART; z <= NZ + ZSTART - VECLEN; z += VECLEN) {
                    ...
                    vload(v_y_minus_1, B[(t+1)%2][x                ][y-YSLOPE][z]);
                    vload(v_y_plus_1 , B[(t+1)%2][x                ][y+YSLOPE][z]);
                    ...
                    vload(v_y_minus_1, B[(t  )%2][x+STRIDE         ][y-YSLOPE][z]);
                    vload(v_y_plus_1 , B[(t  )%2][x+STRIDE         ][y+YSLOPE][z]);
                    ...
                    vload(v_y_minus_1, B[(t+1)%2][x+STRIDE*2       ][y-YSLOPE][z]);
                    vload(v_y_plus_1 , B[(t+1)%2][x+STRIDE*2       ][y+YSLOPE][z]);                    
                    ...
                    vload(v_y_minus_1, B[(t  )%2][x+STRIDE*3       ][y-YSLOPE][z]);
                    vload(v_y_plus_1 , B[(t  )%2][x+STRIDE*3       ][y+YSLOPE][z]);
			...
			for ( z = ZSTART ; z <= NZ + ZSTART - VECLEN; z += VECLEN){	
                ...
                vload(v_y_minus_1, B[t%2    ][x - XSLOPE + STRIDE *3][y + NY + YSLOPE][z]); //y = NY + YSTART
				vload(v_y_plus_1 , B[(t+1)%2][x - XSLOPE + STRIDE *2][y + NY + YSLOPE][z]);
				vload(v_x_minus_1, B[t%2    ][x - XSLOPE + STRIDE *1][y + NY + YSLOPE][z]);
				vload(v_x_plus_1 , B[(t+1)%2][x - XSLOPE            ][y + NY + YSLOPE][z]);
				...
				vstore( B[t%2    ][x - XSLOPE + STRIDE *3][y + NY + YSLOPE][z], v_x_plus_1 );   //y = NY + YSTART
				vstore( B[(t+1)%2][x - XSLOPE + STRIDE *2][y + NY + YSLOPE][z], v_x_minus_1);
				vstore( B[t%2    ][x - XSLOPE + STRIDE *1][y + NY + YSLOPE][z], v_y_plus_1 );
				vstore( B[(t+1)%2][x - XSLOPE            ][y + NY + YSLOPE][z], v_y_minus_1);
			}
        }
        ////when x  = NX + VECLEN - STRIDE * VECLEN + 1,
        ////need extra transpose back to memory
        
        // for(x-=1; x < NX + XSTART - STRIDE * VECLEN + 1 + 3; x++){ // when not using inner transpose x start with x-=1
        for(x; x < NX + XSTART - STRIDE * VECLEN + 1 + 3; x++){           
			for ( y = YSTART - YSLOPE ; y <= NY + YSTART ; y ++ ){
                for ( z = ZSTART; z <= NZ + ZSTART - VECLEN; z += VECLEN) {
				    vload(v_center_3, B[t%2    ][x - XSLOPE + STRIDE *3][y][z]);
				    vload(v_center_2, B[(t+1)%2][x - XSLOPE + STRIDE *2][y][z]);
